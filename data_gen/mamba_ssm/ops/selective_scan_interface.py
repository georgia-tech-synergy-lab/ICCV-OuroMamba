...
# Modified from original implementation for OuroMamba
...
def state_fusion(h, delta):
    
    h_new = h[:,:,:,1:] # Ignoring CLS Dimension

    h_new = rearrange(h_new, "b d e (h w) -> b (d e) h w", h=int(h.shape[-1] ** 0.5), w=int(h.shape[-1] ** 0.5))
    delta = rearrange(delta, "b (h w) -> b h w", h=int(h.shape[-1] ** 0.5), w=int(h.shape[-1] ** 0.5))

    # Pad h_new and delta to ensure valid convolution across borders
    h_new_padded = F.pad(h_new, (1, 1, 1, 1), mode='replicate')

    # Extract local 3×3 patches from h_new_padded using unfold
    patches = F.unfold(h_new_padded, kernel_size=3, padding=0)  # (B, 9*C, H*W)
    patches = patches.view(h_new.shape[0], h_new.shape[1], 9, h_new.shape[2], h_new.shape[3])  # (B, C, 9, H, W)

    # Expand delta to match the patch shape and extract 3×3 delta weights per pixel
    delta_padded = F.pad(delta, (1, 1, 1, 1), mode='replicate')  # (B, H+2, W+2)
    delta_patches = F.unfold(delta_padded.unsqueeze(1), kernel_size=3, padding=0)  # (B, 9, H*W)
    delta_patches = delta_patches.view(delta.shape[0], 1, 9, delta.shape[1], delta.shape[2])  # (B, 1, 9, H, W)

    # Apply spatially varying convolution: Element-wise multiplication and sum over kernel dimension
    h_new = (patches * delta_patches).sum(dim=2)  # Sum over the 3×3 dimension
    h_new = rearrange(h_new, "b (d e) h w -> b d e (h w)")
    
    h = torch.cat((h[:,:,:,0], h_new), dim=-1) # Reattach CLS
    
    return h

def compute_attn_matrix_fused(delta, delta_bias, A, B, C, L, x_shape, dtype=torch.float16):
    dt = F.softplus(delta + delta_bias.unsqueeze(0).unsqueeze(-1))
    
    # Compute dA and dB
    dA = torch.exp(torch.einsum("bdl,dn->bldn", dt, A))  # (B, D, L, N)
    dB = torch.einsum("bdl,bnl->bldn", dt, B.squeeze(1))  # (B, D, L, N)

    # Move L to the last axis before reshaping
    dA = dA.permute(0, 2, 3, 1)  # (B, D, N, L)

    # Extract tensor dimensions
    B_size, D_size, N_size, L_size = dA.shape  # (B, D, N, L)

    # Compute H, W from L
    H = W = int(L_size ** 0.5)  # Assuming L is a perfect square
    assert H * W == L_size, "L should be a perfect square to form 2D patches"

    # Reshape L into (H, W)
    dA_reshaped = dA.view(B_size, D_size, N_size, H, W)  # (B, D, N, H, W)

    dA_padded = F.pad(dA_reshaped, (1, 1, 1, 1), mode='replicate')

    # Extract 3×3 patches using unfold
    dA_patches = F.unfold(dA_padded.view(B_size * D_size * N_size, 1, H + 2, W + 2), kernel_size=3, stride=1)
    dA_patches = dA_patches.view(B_size, D_size, N_size, 9, H, W)  # (B, D, N, 9, H, W)

    # Compute product of all values in each 3x3 patch
    dA_modified = dA_patches.prod(dim=3)  # Product over 3×3 neighborhood

    # Reshape back to (B, D, N, L)
    dA_final = dA_modified.view(B_size, D_size, N_size, L_size)

    # Permute back to original order (B, L, D, N)
    dA_final = dA_final.permute(0, 3, 1, 2)

    # Initialize Attention Matrix
    AttnMatrixOverCLS = torch.zeros((x_shape[0], x_shape[1], x_shape[2], x_shape[2]), requires_grad=True).to(dtype).to(dA.device)  # (B, L, L, L)

    # Compute Attention Matrix
    for r in range(L):
        for c in range(r + 1):
            curr_C = C[:, :, :, r]
            currA = torch.ones((dA_final.shape[0], dA_final.shape[2], dA_final.shape[3]), requires_grad=True, dtype=dtype).to(dA.device)
            if c < r:
                for i in range(r - c):
                    currA = currA * dA_final[:, r - i, :, :]
            currB = dB[:, :, c, :]
...
def selective_scan_ref(u, delta, A, B, C, D=None, z=None, delta_bias=None, 
    ...
    delta_average = delta.sum(dim=1)
    delta_average = (delta_average - delta_average.min(dim=0, keepdim=True)[0]) / \
                    (delta_average.max(dim=0, keepdim=True)[0] - delta_average.min(dim=0, keepdim=True)[0])
    ...
    for i in range(u.shape[2]):
        ...
        if i == u.shape[2] - 1:
            last_state = x
        ys.append(y.real * 2 if y.is_complex() else y)

    y = torch.stack(ys, dim=2)          # (B, D, L)
    X_all = torch.stack(X, dim=3)       # (B, D, N, L)
    X_enhanced = state_fusion(X_all, delta_average)
    ...
    if return_last_state:
        return out, last_state, X_enhanced
    else:
        return out, X_enhanced
...
# def selective_scan_fn(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
#                      return_last_state=False, Q_Act=None):
#     """if return_last_state is True, returns (out, last_state)
#     last_state has shape (batch, dim, dstate). Note that the gradient of the last state is
#     not considered in the backward pass.
#     """

#     return selective_scan_ref(u, delta, A, B, C, D, z, delta_bias, delta_softplus, return_last_state, Q_Act=Q_Act)
#     # return SelectiveScanFn.apply(u, delta, A, B, C, D, z, delta_bias, delta_softplus, return_last_state)
...
#     out_proj_weight, out_proj_bias,
#     A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
#     C_proj_bias=None, delta_softplus=True, Q_Act=None,
# ):
#     L = xz.shape[-1]
#     delta_rank = delta_proj_weight.shape[1]
#     d_state = A.shape[-1] * (1 if not A.is_complex() else 2)
...
#     x, z = xz.chunk(2, dim=1)
#     conv1d_bias = conv1d_bias.contiguous() if conv1d_bias is not None else None
#     x = causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias,None, True)
#     # x = causal_conv1d_fn(x, rearrange(conv1d_weight, "d 1 w -> d w"), conv1d_bias, "silu")
#     # We're being very careful here about the layout, to avoid extra transposes.
#     # We want delta to have d as the slowest moving dimension
#     # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
...
#         else:
#             C = rearrange(C, "(b l) (dstate two) -> b dstate (l two)", l=L, two=2).contiguous()
#     y = selective_scan_fn(x, delta, A, B, C, D, z=z, delta_bias=delta_bias, delta_softplus=True, Q_Act=Q_Act)
