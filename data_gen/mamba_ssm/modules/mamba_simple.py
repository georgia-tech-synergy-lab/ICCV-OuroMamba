    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v1":
                ...
                A_b = -torch.exp(self.A_b_log.float())
                out, xai_a = mamba_inner_fn_no_out_proj(
                    xz,
                    self.conv1d.weight,
                    self.conv1d.bias,
                    self.x_proj.weight,
...
#             elif self.bimamba_type == "v2":
#                 A_b = -torch.exp(self.A_b_log.float())
#                 out, self.xai_a = mamba_inner_fn_no_out_proj(
#                     xz,
#                     self.conv1d.weight,
#                     self.conv1d.bias,
#                     self.x_proj.weight,
...
#                     compute_attn_matrix = self.compute_attn_matrix,
#                 )
#                 out_b, self.xai_b = mamba_inner_fn_no_out_proj(
#                     xz.flip([-1]),
#                     self.conv1d_b.weight,
#                     self.conv1d_b.bias,
#                     self.x_proj_b.weight,
...
#                 )
#                 if self.compute_attn_matrix:
#                     self.attn_matrix_a = self.xai_a["attention_matrix"]
#                     self.attn_matrix_b = self.xai_b["attention_matrix"].flip([-1,-2])
#                 else:
#                     xai_vector = self.xai_a["xai_vector"]
#                     xai_vector[:,:,99:] = self.xai_b["xai_vector"].flip([-1])[:,:,99:]
#                     self.xai_b = xai_vector
