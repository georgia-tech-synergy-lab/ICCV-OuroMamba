...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj, delta_proj,
                ...
                Q_ActA = None, Q_ActB = None, Q_ActC = None, Q_ActD = None, Q_Actd = None, layer_idx= None, checkpoint_lvl=1):
        """
             xz: (batch, dim, seqlen)
        """
        ...
        '''
            ...
            scan_intermediates : [ B, D, 1, L]
        '''
        if Q_ActA is not None:
            d, n = A.shape
            A = rearrange(A, "(b l d) n -> b l (d n)", b=1, l=1).contiguous()
            A = Q_ActA(A)
            A = rearrange(A, "b l (d n) -> (b l d) n ", b=1, l=1, d=d, n=n).contiguous()
            
        if Q_ActB is not None:
            b, _, n, l = B.shape
            B = rearrange(B, "b a n l -> b l (a n)", b=b, a=1, n=n, l=l).contiguous()
            B = Q_ActB(B)
            B = rearrange(B, "b l (a n) -> b a n l", b=b, a=1, n=n, l=l).contiguous()
        
        if Q_ActC is not None:
            b, _, n, l = C.shape
            C = rearrange(C, "b a n l -> b l (a n)", b=b, a=1, n=n, l=l).contiguous()
            C = Q_ActC(C)
            C = rearrange(C, "b l (a n) -> b a n l", b=b, a=1, n=n, l=l).contiguous()
        
        if Q_ActD is not None:
            D = D.view(1, 1, D.shape[0])
            D = Q_ActD(D)
            D = D.view(-1)
        ...
        mean = 0
        if Q_Actd is not None:
            # org_delta = delta.clone()
            b, d, l = delta.shape
            delta = rearrange(delta, "b d l -> b l d", b=b, l=l, d=d).contiguous()
            delta = Q_Actd(delta)
            delta = rearrange(delta, " b l d -> b d l ", b=b, l=l, d=d).contiguous()
        ...
        if Q_Act is not None:
            # og_scan = scan_intermediates.clone()
            _, d, n, _ = scan_intermediates.shape
            scan_intermediates = rearrange(scan_intermediates, "b d n l -> b l (d n)").contiguous()
            scan_intermediates = Q_Act(scan_intermediates)
            scan_intermediates = rearrange(scan_intermediates, "b l (d n) ->b d n l ", d=d, n=n).contiguous()
    ...
    def backward(ctx, dout):
        # print("MambaInnerFnNoOutProj backward")
...
def mamba_inner_fn_no_out_proj(
    ...
    A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True, Q_Act=None
    , Q_ActA=None, Q_ActB=None, Q_ActC=None, Q_ActD=None, Q_Actd=None, layer_idx=None
):  
        
    return MambaInnerFnNoOutProj.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus, Q_Act,
                              Q_ActA, Q_ActB, Q_ActC, Q_ActD, Q_Actd, layer_idx)
